...
- **Handler.** Children started before `t` get a new restart generation, and those started after keep theirs. A pending session that failed before `t` has its deadline moved to `t` while its attempt count survives; one that failed after `t` keeps its whole cool-off. A restarted pane's leftover pending entry is gone, so a provider flipping to `.stale` right afterwards cannot unmount it. The attach recency order is unchanged when only some panes restart. An auth-needed session is still blocked after the event.

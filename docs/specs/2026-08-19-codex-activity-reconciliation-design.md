...
A successfully recreated Codex window is a deliberate fresh-process boundary, not a partial-history row. After the replacement window exists, TBD atomically stores its new tmux coordinates, clears the dead process's session identity, transcript path, session-order watermark, attention, and activity facts, and retains the tab's Codex label and kind. The replacement process's first SessionStart is therefore an initial attachment and may adopt lifecycle records it wrote before its hook arrived. Until that start arrives, session-bound activity hooks from the dead process mismatch the cleared identity and change nothing. A failed recreation does not clear the old row.

...
absence: the desk reads its window through `probeWindow`, whose `TmuxPresence` keeps
`.absent` (tmux answered that the window is gone) apart from `.unknown` (tmux could not
be asked), and only `.absent` may lead to a spawn. This document covers the second: even with the classification correct, a desk
...
## Who reclaims the replacements

A replacement is not a new kind of resource and does not take a new creation path. It
is the same desk terminal the desk's first spawn creates, made by the same
`WorktreeLifecycle.spawnPrimaryTerminals` call, and it gets a `Terminal` row in the desk

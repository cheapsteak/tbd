...
- **Land** — each precondition failure is reported without creating a worktree; with `forks: true` a second landing gets a suffixed branch from the same commit, and with `forks: false` no second landing is offered at all; a `branch` beginning with `-` and an `ext::`-style `remote_url` are both rejected before reaching git.

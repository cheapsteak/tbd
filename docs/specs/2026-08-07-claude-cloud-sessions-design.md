...
**The shared domain type is where the work actually is.** `WorktreeLocation` is `.local` | `.remote(provider:sessionID:)`, and `.local` carries no associated values, so a landed lane — local files, provider origin — has no representation in the model at all. Worse, the current mapping destroys it in both directions rather than merely failing to express it: `WorktreeRecord.init(from:)` sets `providerName` and `providerSessionID` to nil whenever location is `.local`, so writing a landed row through the domain model erases its origin; and `toModel()` rebuilds `.remote(...)` only when the stored location string is `"remote"` and both fields are present, dropping the pair otherwise, so reading it back discards what survived. A landed row would round-trip clean.

**Provenance should become its own field rather than a payload on `.local`.** The two facts are orthogonal — where a lane's files are now, and which session it came from — and the reason they cannot both be expressed today is exactly that the enum conflates them. An optional origin field alongside `location` on `Worktree` keeps `.local` payload-free, so every existing `switch` over `WorktreeLocation` across the daemon and app compiles untouched, and only the record mapping and the model itself change. Adding associated values to `.local` would express the same fact while forcing every match site to be revisited, which is the more invasive answer to a smaller question.

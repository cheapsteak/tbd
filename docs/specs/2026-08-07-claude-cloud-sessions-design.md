...
Ten minutes is a deliberate choice and the number is stated here rather than left to implementation, because both directions of getting it wrong are user-visible. Too short and a slow-provisioning session — capacity is allocated on demand, and a setup script runs before the session is usable — is declared failed while it is still coming up, stranding a real session outside TBD's inventory. Too long and a create that genuinely failed sits as `pending` in front of the user with nothing to act on. Ten minutes is several discovery cycles past the point where a healthy create has appeared, which is the property that matters; it is a threshold to revise on evidence, not a constant to defend.

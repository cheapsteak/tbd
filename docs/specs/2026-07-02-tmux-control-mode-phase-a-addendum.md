...
- **Command length cap:** tmux crashes on very long commands (historically >1024 bytes). Chunk input batches so each `send-keys -H` stays under ~1000 command characters (≈330 input bytes at 3 chars/byte). Large pastes never approach this: a paste over 4 KB is intercepted at the SwiftTerm **view level** — BEFORE SwiftTerm brackets the content — and shipped as a dedicated `.paste` frame on the same sidecar (`SidecarFrameType.paste`, sharing the `.input` payload sub-format). The daemon runs it through the correlator as `load-buffer` + `paste-buffer -d -p`, so it is **ordered with keystrokes end-to-end**: a keystroke enqueued after a paste is FIFO-behind the paste-buffer command through the one `-CC` stream. `-p` hands bracketed-paste wrapping to **tmux**, the single authority on the pane's mode — because the app intercepts before SwiftTerm adds markers, the payload arrives bare and tmux wraps it with `ESC[200~`/`ESC[201~` iff the pane enabled bracketed paste (verified live: a DECSET-2004 pane gets wrapped, a bare pane does not). The paste has a documented cap (`SidecarFrameCodec.maxPasteBytes`, ≈4 MiB — under the scanner's 4 MiB hard cap with header/framing headroom); a paste larger than the cap is **not** split across frames but falls back to SwiftTerm's normal keystroke-path paste (correct, just slower). Pastes ≤ 4 KB are also NOT intercepted — SwiftTerm's three-`send()` bracketed sequence (`ESC[200~` / content / `ESC[201~`) is correct and ordered when it all rides the keystroke path.

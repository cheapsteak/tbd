...
- `meta` — string→string display pairs, provider-defined. The contract blesses `repo`, `branch`, and `profile` as well-known keys a caller MAY interpret; TBD acts on exactly one of them today — `repo` drives the sidebar repo-matching in Part 2. A caller shows only the keys it uses, where it uses them — TBD shows `repo` and `branch` on the provider desk — and does not render unrecognized keys in the session pane.

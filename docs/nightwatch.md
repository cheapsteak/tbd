...
- Person-specific first names, more than one of them. `<operator>` appears 17 times (tier table `:47`, `queue/for-<operator>.md` throughout `tick.py`/`judge.py`, "`%352` = `<operator>`'s own monitoring session" in `dont_touch.txt`, `:1524`), plus "needs `<operator>`'s attention" in the wrap-up prompt (`NightwatchDeskPrompts.swift:81`). A second individual's first name is baked into the standing-rules headers and their surrounding prose (`NightwatchDeskPrompts.swift:41`, `NightwatchSkillContent.swift:111`, `:477`, `:726`) — so the count above covers one name, not every person the sources name.

...
**What bridges a session is the `peer` line on the `messages` stream, not this field.** TBD bridges one of a provider's sessions when a `peer` line names it in `session` and that id resolves to a worktree row TBD already has, and it speaks to that peer at the `protocol` the same line carries. Both are downstream of the caller having peer messaging turned on at all: in TBD that is an off-by-default setting, and while it is off no `messages` stream is opened and nothing is bridged, whatever any session declares or any `peer` line says. The Session object takes part in neither decision. So a session that declares `peer_messaging` is bridged only once a `peer` line announces it, and a session that never declares it is bridged all the same the moment one does: declaring the field enables nothing and omitting it suppresses nothing.

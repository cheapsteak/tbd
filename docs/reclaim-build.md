...
**The reaper never enumerates a repo root itself** — only TBD-managed worktrees (`tbd worktree list`) and, for each derived repo root, the `<root>/.claude/worktrees/*/` glob. A repo root's own `.build` is therefore outside its reach entirely, no matter how stale. Worth stating because the opposite is a natural guess: issue #677 attributed a partially-deleted `.build/checkouts` entry in a repo root to a disk-reclaim sweep, when the reaper had never once targeted that path (zero occurrences across the full `~/Library/Logs/tbd-reclaim-build.log`) and the real cause was SwiftPM's own non-atomic checkout removal.

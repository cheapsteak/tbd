...
- **Installs** — `node_modules`, `.venv`, `.terraform` idle > 48h AND no live Claude session → deleted (regenerate with npm/uv/terraform). NOT gated on `Package.swift`, covering non-Swift worktrees where these dominate disk (typically ~4.3 GB vs ~2.5 GB `.build`). Idleness measured on the install dir's own mtime (cheap single stat, no deep scan).

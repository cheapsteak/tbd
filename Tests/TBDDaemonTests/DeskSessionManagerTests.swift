...
/// Mutable set of tmux windows that `probeWindow` should report as gone. Mutable rather
...
private final class DeadWindows: @unchecked Sendable {
    ...
    /// tri-state `dryRunWindowPresence` seam can express this; the older
    ...
    func presence(_ windowID: String) -> TmuxPresence {
        lock.lock(); defer { lock.unlock() }
        if unverifiable.contains(windowID) { return .unknown }
        return dead.contains(windowID) ? .absent : .alive
    }
}
...
extension TBDHomeSerialized {
    ...
    struct DeskSessionManagerTests {
        ...
        private func makeDeskFixture(tag: String) throws -> (
            ...
            let manager = DeskSessionManager(
                ...
                tmux: TmuxManager(
                    ...
                    dryRunWindowPresence: { _, windowID in dead.presence(windowID) }
                ),
        ...
        /// The regression: a failed identity query dropped the candidate (correct
        /// for "don't paste into a pane I can't verify"), but the spawn logic
        /// reused that to mean "desk is unstaffed" and spawned every tick.
        ...
        func testConcurrentTerminalIsNotAdoptedAsTheReplacement() async throws {
            ...
            let manager = DeskSessionManager(
                ...
                tmux: TmuxManager(
                    ...
                    dryRunPaneCurrentCommand: { _, paneID in f.commands.command(for: paneID) },
                    dryRunPaneSendTarget: { _, paneID in try f.identities.answer(for: paneID) },
                    dryRunWindowPresence: { _, windowID in f.dead.presence(windowID) }
                ),
                skillDir: f.home.appendingPathComponent("skills/nightwatch").path,
                actuationLog: makeTestActuationLog()
            )
        ...
        /// A failed consultation is not the same as a genuine "pane is gone"
        /// answer. When the identity consultation cannot be run (tmux times out
        /// and `paneSendTarget` throws), the candidate reads `.unknown`, which
        /// bars the paste and must not license a spawn — "cannot tell" is not
        /// "empty".

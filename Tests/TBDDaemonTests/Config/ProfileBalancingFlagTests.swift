...
/// Schema and resolution guards for `config.profile_balancing_enabled`,
/// the soak gate for spreading new sessions across profiles with the most room
/// (design 2026-09-05 §6).
///
/// The column is added by `20260905080315_config_profile_balancing` with
...
@Suite("ProfileBalancingFlag")
struct ProfileBalancingFlagTests {
    ...
    @Test func nullBeforeAnyGesture() async throws {
        ...
        #expect(
            record.profile_balancing_enabled == nil,
            """
            config.profile_balancing_enabled must be NULL until the toggle \
            is touched — read back \
            \(String(describing: record.profile_balancing_enabled)). A non-nil \
            value here means 20260905080315_config_profile_balancing grew a \
    ...
    @Test func rowWrittenBeforeTheMigrationStillReadsNull() throws {
        ...
        try queue.read { db in
            let row = try #require(try Row.fetchOne(
                ...
                arguments: [ConfigStore.singletonID]))
            let raw: DatabaseValue = row["profile_balancing_enabled"]
            #expect(
                raw.isNull,
    ...
    /// NULL follows `Config.profileBalancingEnabledDefault` wherever it
    ...
    @Test func explicitFalseSurvivesADefaultFlipWhileNullFollowsIt() async throws {
        ...
        #expect(untouched.profile_balancing_enabled == nil)
        #expect(
            untouched.toModel(profileBalancingDefault: false)
                .profileBalancingEnabled == false)
        #expect(
            untouched.toModel(profileBalancingDefault: true)
                .profileBalancingEnabled == true,
        ...
        #expect(explicitlyOff.profile_balancing_enabled == false)
        #expect(
            explicitlyOff.toModel(profileBalancingDefault: false)
                .profileBalancingEnabled == false)
        #expect(
            explicitlyOff.toModel(profileBalancingDefault: true)
                .profileBalancingEnabled == false,
            "an explicit opt-out must be honored forever, whatever the shipped default becomes")
    }

    /// Mirrored for an explicit `true`: an operator who opted into the balancing
    ...
    @Test func explicitTrueSticks() async throws {
        ...
        #expect(explicitlyOn.profile_balancing_enabled == true)
        #expect(
            explicitlyOn.toModel(profileBalancingDefault: false)
                .profileBalancingEnabled == true)
        #expect(
            explicitlyOn.toModel(profileBalancingDefault: true)
                .profileBalancingEnabled == true)
    }

    /// Isolates the RESOLUTION guard (`toModel()`'s
    /// `?? profileBalancingDefault`) from the STORAGE guard (the
    ...
    @Test func toModelResolvesNullThroughTheInjectedDefault() {
        let record = ConfigRecord(id: "unstored", profile_balancing_enabled: nil)
        #expect(
            record.toModel(profileBalancingDefault: false)
                .profileBalancingEnabled == false)
        #expect(
            record.toModel(profileBalancingDefault: true)
                .profileBalancingEnabled == true,
    ...
    @Test func configJSONWithoutTheKeyFollowsTheShippedDefault() throws {
        ...
        #expect(
            config.profileBalancingEnabled == Config.profileBalancingEnabledDefault)
    }
    ...
    @Test func configJSONRoundTripsAnExplicitChoice() throws {
        var config = Config()
        config.profileBalancingEnabled = true
        let decoded = try JSONDecoder().decode(
            Config.self, from: JSONEncoder().encode(config))
        #expect(decoded.profileBalancingEnabled == true)
    }
}

...
/// Schema and resolution guards for `config.gc_holder_rendezvous_enabled`, the
/// soak gate on the sweep that unlinks a dead pty holder's socket, lock and log.
///
/// The column is added by `20260901135111_config_gc_holder_rendezvous` with **no
...
/// phase unlinks files: somebody who turned it off did so deliberately and must
/// stay opted out through graduation.
@Suite("GCHolderRendezvousFlag")
struct GCHolderRendezvousFlagTests {
    ...
    private static let lastIdentifierBeforeTheFlag = "20260831200151_config_holder_owner_token"
    ...
    @Test func nullBeforeAnyGesture() async throws {
        ...
        #expect(
            record.gc_holder_rendezvous_enabled == nil,
            """
            config.gc_holder_rendezvous_enabled must be NULL until the toggle is \
            touched — read back \
            \(String(describing: record.gc_holder_rendezvous_enabled)). A non-nil \
            value here means 20260901135111_config_gc_holder_rendezvous grew a \
    ...
    @Test func rowWrittenBeforeTheMigrationStillReadsNull() throws {
        ...
        try queue.read { db in
            let row = try #require(try Row.fetchOne(
                ...
                arguments: [ConfigStore.singletonID]))
            let raw: DatabaseValue = row["gc_holder_rendezvous_enabled"]
            #expect(
                raw.isNull,
    ...
    /// NULL follows `Config.gcHolderRendezvousEnabledDefault` wherever it goes;
    ...
    @Test func explicitFalseSurvivesADefaultFlipWhileNullFollowsIt() async throws {
        ...
        #expect(untouched.gc_holder_rendezvous_enabled == nil)
        #expect(
            untouched.toModel(gcHolderRendezvousDefault: false).gcHolderRendezvousEnabled == false)
        #expect(
            untouched.toModel(gcHolderRendezvousDefault: true).gcHolderRendezvousEnabled == true,
        ...
        #expect(explicitlyOff.gc_holder_rendezvous_enabled == false)
        #expect(
            explicitlyOff.toModel(gcHolderRendezvousDefault: false)
                .gcHolderRendezvousEnabled == false)
        #expect(
            explicitlyOff.toModel(gcHolderRendezvousDefault: true)
                .gcHolderRendezvousEnabled == false,
    ...
    @Test func explicitTrueSticks() async throws {
        ...
        #expect(explicitlyOn.gc_holder_rendezvous_enabled == true)
        #expect(
            explicitlyOn.toModel(gcHolderRendezvousDefault: false).gcHolderRendezvousEnabled == true)
        #expect(
            explicitlyOn.toModel(gcHolderRendezvousDefault: true).gcHolderRendezvousEnabled == true)
    }

    /// Isolates the RESOLUTION guard (`toModel()`'s `?? gcHolderRendezvousDefault`)
    ...
    @Test func toModelResolvesNullThroughTheInjectedDefault() {
        let record = ConfigRecord(id: "unstored", gc_holder_rendezvous_enabled: nil)
        #expect(record.toModel(gcHolderRendezvousDefault: false).gcHolderRendezvousEnabled == false)
        #expect(
            record.toModel(gcHolderRendezvousDefault: true).gcHolderRendezvousEnabled == true,
    ...
    /// soaks behind its own switch, and the transport whose residue it reclaims
    /// is itself still behind `ptyHolderEnabled`. Graduation edits this constant
    /// and nothing else.
    @Test func shippedDefaultIsOff() async throws {
        #expect(Config.gcHolderRendezvousEnabledDefault == false)
        let db = try TBDDatabase(inMemory: true)
    ...
    @Test func setterRoundtrips() async throws {
        let db = try TBDDatabase(inMemory: true)
        try await db.config.setGCHolderRendezvousEnabled(true)
        #expect(try await db.config.get().gcHolderRendezvousEnabled == true)
        try await db.config.setGCHolderRendezvousEnabled(false)
        #expect(try await db.config.get().gcHolderRendezvousEnabled == false)
    }
    ...
    @Test func configJSONWithoutTheKeyFollowsTheShippedDefault() throws {
        ...
        #expect(config.gcHolderRendezvousEnabled == Config.gcHolderRendezvousEnabledDefault)
    }
    ...
    @Test func configJSONRoundTripsAnExplicitChoice() throws {
        var config = Config()
        config.gcHolderRendezvousEnabled = true
        let decoded = try JSONDecoder().decode(
            Config.self, from: JSONEncoder().encode(config))
        #expect(decoded.gcHolderRendezvousEnabled == true)
    }
}

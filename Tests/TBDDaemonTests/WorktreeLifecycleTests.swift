...
@Test func testCreateWorktreeAutoGeneratedBranchFixesDisplayName() async throws {
    // CONTRACT: Verifies the fix in Sources/TBDApp/AppState+Worktrees.swift:65.
    // When folder and displayName are the same slug, the daemon reuses that slug
    // for name, keeping name == displayName and hasDefaultDisplayName == true.
    //
    // This is what the fixed app now sends for auto-generated branches:
    // AppState.createWorktree lines 65, 67: `folder: placeholderName, displayName: placeholderName`.
    let (tempDir, repoDir) = try await createTestRepo()
    defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    // Simulate the FIXED app behavior: pass both folder and displayName as the same slug.
    let placeholderName = "20260101-test-otter"
    let wt = try await lifecycle.createWorktree(
        repoID: repo.id,
        folder: placeholderName,
        displayName: placeholderName,
        skipClaude: true
    )

    // CONTRACT: verify the fix works
    #expect(wt.name == placeholderName, "name should be the folder")
    #expect(wt.displayName == placeholderName, "displayName should be what was passed")
...
@Test func testCreateWorktreeExistingBranchWithoutFolderHasDefaultDisplayName() async throws {
    ...
    // When creating from an existing branch, AppState does NOT pass folder
    // (lines 65: `folder: existingBranch == nil ? ... : nil`), but DOES pass
    // displayName explicitly (line 67: `displayName: placeholderName`, which is
    // the branch's local name). The daemon derives folder from the branch name
    // (lines 101-114 in WorktreeLifecycle+Create.swift). This test verifies
    // that when no dedup suffix is needed, name == displayName.
    let (tempDir, repoDir) = try await createTestRepo()
    defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    // Simulate the app's actual input for existing branches: no folder, but
    // displayName is passed explicitly (the branch's local name).
    let wt = try await lifecycle.createWorktree(
        repoID: repo.id,
        folder: nil,
        branch: "my-feature",

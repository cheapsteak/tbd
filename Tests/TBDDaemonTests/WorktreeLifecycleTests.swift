...
@Test func testCreateWorktreeAutoGeneratedBranchMaintainsDefaultDisplayName() async throws {
    // Regression test for: new worktrees are born with `displayName != name`
    // (two independently generated slugs), which defeats the `stop-rename-check`
    // Stop hook (which only fires when `hasDefaultDisplayName`, i.e.
    // `displayName == name`). This test verifies the fix: passing the folder
    // parameter to the daemon ensures the daemon reuses the same slug for both
    // `name` and `displayName` in the auto-generated (non-existing-branch) path.
    let (tempDir, repoDir) = try await createTestRepo()
    defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    // Create a worktree with auto-generated branch (existingBranch == nil).
    let wt = try await lifecycle.createWorktree(repoID: repo.id, skipClaude: true)

    // The key invariant: for auto-generated branches, name must equal displayName
    // so that `hasDefaultDisplayName` is true and the stop-rename-check hook fires.
    #expect(wt.name == wt.displayName, "Auto-generated worktrees must have name == displayName")
    #expect(wt.hasDefaultDisplayName, "Auto-generated worktrees must have hasDefaultDisplayName == true")
...
@Test func testCreateWorktreeExistingBranchWithoutFolderHasDefaultDisplayName() async throws {
    // Companion test: when creating a worktree from an existing branch, the
    // daemon should NOT receive a folder argument, so it derives the folder
    // from the branch name. The displayName is passed explicitly (the branch's
    // local name), and the daemon generates a potentially different folder
    // (with dedup suffix if needed). This test verifies that despite this, the
    // invariant `name == displayName` still holds for the simpler existing-branch
    // case where no suffix is needed.
    let (tempDir, repoDir) = try await createTestRepo()
    defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    #expect(wt.name == "my-feature")
    #expect(wt.displayName == "my-feature")

...
struct ProxyControlTests {
    ...
    func retireAnswersBeforeDrainAndSuccessorCanBind() async throws {
        ...
        try await withProxy(
            ...
            let boundPort = try await withPhaseDeadline("successor bind", seconds: 5) {
                try await successor.start()
            }
            let bindTook = ContinuousClock().now - bindStarted
            #expect(boundPort == harness.port)
            #expect(bindTook < .seconds(1), "the successor waited \(bindTook) for the port")

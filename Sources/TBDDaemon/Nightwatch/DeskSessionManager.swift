...
public actor DeskSessionManager: DeskSessionManaging {
    ...
    /// `probeWindow` proves a window id resolves to SOME window;
    ...
    private func classifyAgentTerminals(
        ...
        for terminal in candidates {
            classification.record(
                terminal,
                try await verdict(server: server, terminal: terminal, kind: kind))
        }
    ...
    ///
    /// The window probe runs first because it is the consultation that keeps
    /// "tmux looked and the window is gone" apart from "tmux could not be asked"
    /// (`probeWindow`'s `TmuxPresence`). Once the window has answered `.alive`,
    /// the server is reachable and an `.absent` or `.dead` pane is a real
    /// answer; a pane read that nonetheless reaches no server throws
    /// `DeskPaneUnreachable` and aborts the pass. The identity and
    /// foreground-command checks still gate `.live` exactly as before, so
    /// nothing reaches a paste that would not have reached one previously.
    ///
    /// - Throws: `DeskPaneUnreachable`, from `paneIdentityVerdict`.
    private func verdict(
        server: String,
        terminal: Terminal,
        kind: TerminalKind
    ) async throws -> CandidateVerdict {
        ...
        case .absent:
            ...
            return .absent
        case .unknown:
            // Same rule as the other two consultations. The `Bool` probe folds a
            // wedged or unreachable server into "the window is gone", which is
            // the exact collapse that licenses a spawn beside a healthy judge;
            // `probeWindow` keeps "tmux could not answer" apart.
            logger.notice("""
        ...
        let identity = try await paneIdentityVerdict(server: server, terminal: terminal)
    ...
    private func paneIdentityVerdict(
        ...
        case .unreachable:
            ...
            // licenses a SPAWN of a replacement agent, and makes
            // `leasedJudgeTerminal` REVOKE a valid lease. Doing either because a
            // read failed would duplicate a live desk. So this does not answer
            // the question at all — it throws, which aborts the whole pass
            // before any of those act, and the callers' existing `catch` logs
            // and skips the tick.
            logger.warning("""
    ...
    private func deskStaffing(worktree: Worktree) async -> CandidateClassification {
        ...
        for kind in [TerminalKind.claude, .codex] {
            do {
                ...
                // The row read itself failed, or a pane read reached no tmux
                // server (`DeskPaneUnreachable`), so this desk has not been
                // ruled empty — the same rule the per-pane consultations
                // follow. The `rowsUnreadable` sentinel carries that through
                // `isStaffed` without inventing a Terminal nobody read.
                logger.warning("""
                    Watch Desk staffing unknown: could not classify \
                    \(kind.rawValue, privacy: .public) terminals: \
    ...
    public func nudgeDeskSession(worktreeID: UUID, act: Bool) async {
        ...
        do {
            ...
            // success, and the backstop would never reach its cap.
            resetRecoveryBudget()

...
extension AppState {
    ...
    /// mirror, not `gone`, and whose provider declares the `attach`
    /// capability. Reuses `RemoteSessionDetailGates.available` — the exact
    /// same gate that decides whether the Attach TAB even renders — so a
    /// provider without the capability, or a gone session, can never end up
    /// attach-eligible here while simultaneously having no Attach tab to
    /// show it in (the two must never disagree).

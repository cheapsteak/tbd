...
The update clone at `~/tbd/updates/src` sits outside all three on purpose. It is one long-lived directory rather than a per-request resource, so it cannot accumulate: every update reuses it, `scripts/reclaim-build.sh` never scans it, and it appears in nobody's `git worktree list`. Removing it is an operator gesture, and the next update clones it again — see [`docs/updating.md`](docs/updating.md).

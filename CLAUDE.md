...
Integration-style tests that exercise production code paths which internally read `TBD_HOME` — and thus cannot accept an explicit env dict — may use `setenv("TBD_HOME", "/some/tmp/dir", 1)` (and optionally `TBD_SOCKET_PATH`); for all other cases prefer the injection-seam approach, and see the next paragraph for where `setenv` is permitted. Tests that mutate `UserDefaults` mint their suite through `TestDefaultsSuite` / `withTestDefaults` (`Tests/TestSupport/UserDefaultsTestSupport.swift`) and never call `UserDefaults(suiteName:)` directly — `UserDefaults.standard` on this unbundled executable is the developer's real `TBDApp.plist`. **`removePersistentDomain(forName:)` is not a teardown.** It clears the domain's values and leaves the backing file behind — a 42-byte `bplist00` husk, forever — so the previous rule, which prescribed exactly that, leaked ~520,000 orphaned plists (~2.1 GB) into `~/Library/Preferences` before anyone counted them. The helper removes the domain, forces the flush, and unlinks the file; it also names every suite `TBDTests.suites/<label>.<uuid>` so the backing file lands in one container directory (`~/Library/Preferences/TBDTests.suites`) that can be reclaimed wholesale. That container is not a tidiness measure: `cfprefsd` caches the domain and re-flushes it to disk when the process exits, strictly after every `atexit` handler, so no in-process unlink order can be the last word — measured, six orderings, all leaked.
...
**Run tests with `scripts/test.sh`, not bare `swift test`.** It forwards its arguments to `swift test` behind two layers. The **fence** (always on) points `TBD_HOME`, `TBD_SOCKET_PATH`, `TBD_CLAUDE_HOST_HOME`, `TBD_TEST_CODEX_HOME` and `TMUX_TMPDIR` at a scratch dir, and `HOME`/`CFFIXED_USER_HOME` at a second one whose `tbd`, `.claude` and `.codex` entries are mode `000`. The two host-store legs matter as much as the first: a default-constructed `ClaudeProfileConfigDirManager` otherwise gets a scratch profiles dir and the developer's real `~/.claude` as the host store it moves subtrees around in, and `CodexHomeManager` falls back to the real `~/.codex` and writes a plugin, hooks and a profile TOML into it. `TMUX_TMPDIR` fences the shared `/tmp/tmux-<uid>`, which every `-L <name>` resolves under, and it is the one leak no teardown can undo: **tmux never unlinks its socket file when a server exits** — it unlinks a stale one lazily when a new server claims that exact path, and every test mints a fresh name, so ~7,100 dead sockets accumulated in nine days. Keep the fenced directory directly under the short `/tmp` scratch root or the socket path outgrows `sun_path`. The **detector** fingerprints the real `~/tbd`, `~/.claude`, `~/.codex`, the tmux socket directory and `~/Library/Preferences` on either side and fails the run if any entry appeared or vanished; **the fifth root is asymmetric and that asymmetry is the point** — it has a detector arm but no fence half, because `cfprefsd` resolves preferences over XPC and honours neither `HOME` nor `CFFIXED_USER_HOME`, so nothing the wrapper exports can redirect a `UserDefaults` write. That is exactly why the plist leak above was invisible for months while the four fenced roots stayed clean. In place of a fence half it gets a **reclaim**: the wrapper deletes the exact path `~/Library/Preferences/TBDTests.suites` before the run and again after it, pairing with the helper's own start-of-process sweep so the container is empty on both sides of the bracket — without the after-the-run half the two snapshots straddle different runs' leftovers and the arm reddens every time. The reclaim is not a gate and never a glob: it refuses any path not ending in the container name, refuses a symlink or a non-directory, and always succeeds. Detection is on when `$CI` is set, off otherwise, `--fingerprint` opts in locally and `--no-fingerprint` forces it off — which turns off the report and never the sweep. It is CI-shaped because on a developer box a live daemon, sibling worktrees and concurrent agent sessions write to those directories legitimately throughout a run, and a guard that reddens spuriously gets disabled. The rule above had been in this file all along and the suite violated it anyway — 18k orphan profile dirs and ~2.9k fake worktrees accumulated unnoticed — because the invariant belongs to a whole run and no per-test assertion can express it. The wrapper's guards have their own mutation-checked harness, `scripts/test.test.sh` (~11 s, no build, no real store touched), wired into the `lint` CI job — edit a guard, edit its case.
